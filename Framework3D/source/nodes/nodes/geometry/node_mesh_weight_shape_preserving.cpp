            ...
            for (auto j = 0; j < angles.size(); j++) {
                ...
                if (j != i && next != i) {
                    ...
                    if (sigma1 + sigma2 + sigma3 < 1.001) {
                        result[vertex.idx() * n + vertex.idx()] += 1.0f / S;
                        result[vertex.idx() * n + id_list[i]] += -sigma1 / S;
                        result[vertex.idx() * n + id_list[j]] += -sigma2 / S;
                        result[vertex.idx() * n + id_list[next]] += -sigma3 / S;
                    }
                }
            }
        }
