...
static void node_curvature_exec(ExeParams params)
{
    ...
    pxr::VtArray<float> rst(mesh->n_vertices());
    ...
    for (const auto& vertex_handle : mesh->vertices()) {
        ...
        const auto& position = mesh->point(vertex_handle);
        ...
        //   for (const auto& face_handle : mesh->vf_range(vertex_handle))
        ...
        for (const auto& halfedge_handle : vertex_handle.outgoing_halfedges()) {
            ...
            const auto& vec1 = mesh->point(v1) - position;
            const auto& vec2 = mesh->point(v2) - position;
    ...
    auto operand_base = openmesh_to_operand(mesh.get());
...
static void node_register()
{
    ...
    strcpy_s(ntype.ui_name, "Curvature");
