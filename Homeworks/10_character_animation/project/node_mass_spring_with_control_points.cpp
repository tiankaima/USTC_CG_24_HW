...
static void node_mass_spring_declare(NodeDeclarationBuilder &b) {
  ...
  b.add_input<decl::Int>("time integrator type").default_val(0).min(0).max(1);
  ...
  if (time_code == 0) {
    if (mesh) {
      ...
      auto edges = get_edges(
          usd_faces_to_eigen(mesh->faceVertexCounts, mesh->faceVertexIndices));
      ...
      if (!mass_spring->set_dirichlet_bc_mask(
              VtIntArray_to_vector_bool(fixed_points)))
      ...
      if (!mass_spring->init_dirichlet_bc_vertices_control_pair(
        ...
        throw std::runtime_error("Mass Spring: init control points error.");
    ...
    mass_spring->update_dirichlet_bc_vertices(
        usd_vertices_to_eigen(controller_mesh->vertices));
    mass_spring->compute_sphere(fixed_points);
    mass_spring->step();
  }
