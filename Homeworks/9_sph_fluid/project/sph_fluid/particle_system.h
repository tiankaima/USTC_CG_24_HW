...
public:
  ...
  Eigen::Vector3d acceleration_pressure_;
...
public:
  ParticleSystem(const MatrixXd &X, const Vector3d &box_min,
                 const Vector3d &box_max);

  const std::vector<std::shared_ptr<Particle>> &particles() const {
    return particles_;
  }
  const double h() const { return support_radius_; }

  const double volume() const { return particle_volume_; }
  const double mass() const { return particle_mass_; }
  const double density0() const { return density0_; }
  ...
  void add_particle();
  static MatrixXd
  sample_particle_pos_in_a_box(const Vector3d min, const Vector3d max,
                               const Vector3i n_particle_per_axis);
  unsigned pos_to_cell_index(const Vector3d &x) const;
  unsigned cell_xyz_to_cell_index(const unsigned x, const unsigned y,
