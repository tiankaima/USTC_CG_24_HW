...
ParticleSystem::ParticleSystem(const MatrixXd &X, const Vector3d &box_min,
  ...
  box_min_ = box_min;
  n_cell_per_axis_ =
      ((box_max - box_min) / cell_size_).array().ceil().cast<int>();

  cells_.resize(n_cell_per_axis_[0] * n_cell_per_axis_[1] *
                n_cell_per_axis_[2]);
...
unsigned ParticleSystem::cell_xyz_to_cell_index(const unsigned x,
                                                const unsigned y,
                                                const unsigned z) const {
  return x * n_cell_per_axis_[1] * n_cell_per_axis_[2] +
         y * n_cell_per_axis_[2] + z;
}
...
std::vector<unsigned>
ParticleSystem::get_neighbor_cell_indices(const Vector3d &pos) const {
    ...
    for (int j = -1; j <= 1; j++) {
      for (int k = -1; k <= 1; k++) {
        if (x + i < 0 || x + i >= n_cell_per_axis_[0] || y + j < 0 ||
            y + j >= n_cell_per_axis_[1] || z + k < 0 ||
            z + k >= n_cell_per_axis_[2]) {
          continue;
        }
...
void ParticleSystem::assign_particles_to_cells() {
  ...
  for (auto &p : particles_) {
    ...
    if (cell_idx >= 0 && cell_idx < cells_.size())
      ...
      std::cout << "[assign_particles_to_cells] cell_idx out of range: cell= "
                << cell_idx;
      std::cout << " particle pos= " << p->X_.transpose() << std::endl;
      std::cout << "n cells per axis = " << n_cell_per_axis_.transpose()
                << std::endl;
...
MatrixXd ParticleSystem::sample_particle_pos_in_a_box(
    const Vector3d min, const Vector3d max, const Vector3i n_per_axis) {
